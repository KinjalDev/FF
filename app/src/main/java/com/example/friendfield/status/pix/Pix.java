package com.example.friendfield.status.pix;


import static com.example.friendfield.Activity.TakePhotoActivity.getImageUri;
import static com.example.friendfield.Utils.ImageEditor.EXTRA_IMAGE_PATH;
...
import com.example.friendfield.status.adapters.InstantImageAdapter;
import com.example.friendfield.status.adapters.MainImageAdapter;
import com.example.friendfield.status.interfaces.OnSelectionListener;
import com.example.friendfield.status.interfaces.WorkFinish;
import com.example.friendfield.status.modals.Img;
import com.example.friendfield.status.utility.Constants;
import com.example.friendfield.status.utility.HeaderItemDecoration;
import com.example.friendfield.status.utility.ImageVideoFetcher;
import com.example.friendfield.status.utility.PermUtil;
import com.example.friendfield.status.utility.Utility;
import com.example.friendfield.status.utility.ui.FastScrollStateChangeListener;
...
public class Pix extends AppCompatActivity implements View.OnTouchListener {
    ...
    private static int maxVideoDuration = 15000;
...
//        finish();
        ...
        Log.e("LLL_list-->", list.get(0).toString());
        String select_file = list.get(0).toString();

        String lastWord = select_file.substring(select_file.lastIndexOf(".") + 1);
        Log.e("LLL_f_image_path-->", lastWord);

        Intent intent = new Intent(getApplicationContext(), ImageEditActivity.class);
        intent.putExtra(EXTRA_IMAGE_PATH, list.get(0).toString());
...
//        maxVideoDuration = options.getVideoDurationLimitinSeconds() * 1000; //conversion in  milli seconds
        ...
        colorPrimaryDark =
                ResourcesCompat.getColor(getResources(), R.color.darkturquoise, getTheme());
...
//                            Bitmap img_bitmap = BitmapFactory.decodeFile(photo.getPath());

                            Matrix matrix = new Matrix();
//                            matrix.postRotate(-90);

                            Bitmap front_bitmap = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(),
                                    matrix, true);

                            Bitmap bitmap1 = flip(front_bitmap, FLIP_HORIZONTAL);

                            ByteArrayOutputStream bytes = new ByteArrayOutputStream();
                            bitmap1.compress(Bitmap.CompressFormat.JPEG, 100, bytes);
                            String imgString = String.valueOf(getImageUri(getApplicationContext(), bitmap1));

                            Img img = new Img("", "", imgString, "", 1);
                            selectionList.add(img);
                            Utility.scanPhoto(Pix.this, new File(imgString));
//                            returnObjects();

                            Intent intent = new Intent(getApplicationContext(), ImageEditActivity.class);
//                intent.putExtra(EXTRA_IMAGE_PATH, list.get(0).toString());
                            intent.putExtra(EXTRA_IMAGE_PATH, imgString);
//                if (lastWord.equalsIgnoreCase("jpg") || lastWord.equalsIgnoreCase("jpeg") || lastWord.equalsIgnoreCase("png") || lastWord.equalsIgnoreCase("svg") || lastWord.equalsIgnoreCase("webp")) {
                            intent.putExtra("isFromCamera", true);
                            intent.putExtra(ImageEditor.EXTRA_IS_CROP_MODE, enableEditorCrop);
//                } else {
...
//                returnObjects();
                ...
                Log.e("LLL_cara_face-->", result.getFacing().toString());

                Intent intent = new Intent(getApplicationContext(), ImageEditActivity.class);
//                intent.putExtra(EXTRA_IMAGE_PATH, list.get(0).toString());
                intent.putExtra(EXTRA_IMAGE_PATH, result.getFile().getAbsolutePath());
                intent.putExtra("isFromCamera", false);
                intent.putExtra(ImageEditor.EXTRA_IS_CROP_MODE, false);
                if (result.getFacing() == Facing.FRONT) {
        ...
        flashDrawable = R.drawable.ic_off;
        ...
        clickme.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    ...
                    findViewById(R.id.clickme).setVisibility(View.VISIBLE);
                    ...
                    findViewById(R.id.clickme).setVisibility(View.GONE);
                ...
                iv.animate()
                        ...
                        .setListener(new AnimatorListenerAdapter() {
                            ...
                            public void onAnimationEnd(Animator animation) {
                                ...
                                if (flashDrawable == R.drawable.ic_flashauto) {
                                    flashDrawable = R.drawable.ic_off;
                                    iv.setImageResource(flashDrawable);
                                    camera.setFlash(Flash.OFF);
                                } else if (flashDrawable == R.drawable.ic_off) {
                                    flashDrawable = R.drawable.ic_on;
                                    iv.setImageResource(flashDrawable);
                                    camera.setFlash(Flash.ON);
                                } else {
                                    flashDrawable = R.drawable.ic_flashauto;
                                    iv.setImageResource(flashDrawable);
        ...
        front.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                oa1.addListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        super.onAnimationEnd(animation);
                        front.setImageResource(R.drawable.ic_swich_camera);
                        oa2.start();
                    }
                });
                oa1.start();
